          ...
          {albums.map((album, i) => {
            ...
            return (
              <div key={i} className="group cursor-pointer" onClick={() => {
                // In the future, this could navigate to an Album Details page.
                // For now, we can just play the album when clicking it.
                const albumTracks = tracks.filter(t => t.album === album);
                if (albumTracks.length > 0) {
                  playerState.playTrack(albumTracks[0], albumTracks);
                }
                ...
                <div className={`w-full aspect-square rounded-2xl mb-3 flex items-center justify-center border border-white/5 group-hover:border-[#f59e0b]/40 transition-all relative overflow-hidden bg-gradient-to-br`}
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                    <button onClick={(e) => { e.stopPropagation(); const albumTracks = tracks.filter(t => t.album === album); playerState.addToCurrentQueue(albumTracks); }} className="w-10 h-10 bg-white/20 hover:bg-white/40 rounded-full flex items-center justify-center hover:scale-110 shadow-lg transition-transform mx-1" title="Thêm vào hàng chờ hiện tại">
