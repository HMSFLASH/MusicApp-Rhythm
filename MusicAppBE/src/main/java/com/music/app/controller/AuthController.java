...
public class AuthController {
    ...
    private void setRefreshTokenCookie(HttpServletResponse response, String refreshToken) {
        if (refreshToken != null) {
            response.addHeader(
                    "Set-Cookie",
                    ResponseCookie.from("music_app_refresh_token", refreshToken)
                            .httpOnly(true)
                            .secure(secureCookies)
                            .sameSite("Strict")
    ...
    private void clearTokenCookie(HttpServletResponse response) {
        response.addHeader(
                "Set-Cookie",
                ResponseCookie.from("music_app_token", "")
                        .httpOnly(true)
                        .secure(secureCookies)
                        .sameSite("Strict")
                        .path("/")
    ...
    private void clearRefreshTokenCookie(HttpServletResponse response) {
        response.addHeader(
                "Set-Cookie",
                ResponseCookie.from("music_app_refresh_token", "")
                        .httpOnly(true)
                        .secure(secureCookies)
                        .sameSite("Strict")
                        .path("/api/auth/refresh")
    ...
    public ApiResponse<Void> logout(
            @CookieValue(name = "music_app_token", required = false) String tokenCookie,
            @CookieValue(name = "music_app_refresh_token", required = false) String refreshTokenCookie,
            @RequestHeader(value = "Authorization", required = false) String tokenHeader,
            HttpServletResponse response) {
        String token = tokenCookie != null
                ? tokenCookie
                : (tokenHeader != null && tokenHeader.startsWith("Bearer ") ? tokenHeader.substring(7) : tokenHeader);
        if (token != null) {
    ...
    public ApiResponse<AuthenticationResponse> refresh(
        ...
        if (refreshTokenCookie == null || refreshTokenCookie.isEmpty()) {
            throw new com.music.app.exception.AppException(
                    com.music.app.exception.ErrorCode.UNAUTHENTICATED, "Refresh token is missing");
        }
    ...
    public ResponseEntity<ApiResponse<?>> setPassword(
            @Valid @RequestBody SetPasswordRequest request, HttpServletResponse response) {
        ...
        AuthenticationResponse authResponse =
                authService.setLocalCredentials(currentSubject, request.getLoginId(), request.getPassword());
        setTokenCookie(response, authResponse.getAccessToken());
        setRefreshTokenCookie(response, authResponse.getRefreshToken());
    ...
    public ResponseEntity<ApiResponse<?>> changePassword(
        ...
        String currentSubject = auth.getName();
        AuthenticationResponse authResponse =
                authService.changePassword(currentSubject, request.getOldPassword(), request.getNewPassword());
        setTokenCookie(response, authResponse.getAccessToken());
        setRefreshTokenCookie(response, authResponse.getRefreshToken());
        return ResponseEntity.ok(ApiResponse.builder().result(authResponse).build());
    }
    ...
    public ResponseEntity<ApiResponse<com.music.app.dto.UserDto>> getCurrentUser() {
        ...
        if (auth == null || auth.getName().equals("anonymousUser")) {
            return ResponseEntity.status(401)
                    .body(ApiResponse.<com.music.app.dto.UserDto>builder()
                            .code(401)
                            .message("Not authenticated")
                            .build());
        }
        ...
        return ResponseEntity.ok(
                ApiResponse.<com.music.app.dto.UserDto>builder().result(user).build());
    }
}
